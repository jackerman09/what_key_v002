...
(function() {
  jQuery(function() {
    var allNoteObjects, allNotes, index, note;
    allNotes = [];
    allNoteObjects = $('#addNoteDiv').data('url');
    for (index in allNoteObjects) {
      note = allNoteObjects[index];
      allNotes.push(note.name);
    }
    return $('#addNoteField').autocomplete({
      source: allNotes
    }).focus();
  });
...
(function() {
  jQuery(function() {
    var allChordObjects, allChords, chord, index;
    allChords = [];
    allChordObjects = $('#addChordDiv').data('url');
    for (index in allChordObjects) {
      chord = allChordObjects[index];
      allChords.push(chord.name);
      console.log(allChords);
    }
    return $('#addChordField').autocomplete({
      source: allChords
    }).focus();
  });
...
(function() {
  jQuery(function() {
    var toggleChords, toggleNotes;
    toggleChords = function(e) {
      e.preventDefault();
      if ($(this).text() === 'show chords') {
        $(this).text('hide chords');
      } else {
        $(this).text('show chords');
      }
      return $(this).closest('li').find('.chordsholder').slideToggle();
    };
    toggleNotes = function(e) {
      ...
      return $(this).closest('li').find('.notesholder').slideToggle();
    };
    $('.showremovechords').click(toggleChords);
    return $('.showremovenotes').click(toggleNotes);
  });
